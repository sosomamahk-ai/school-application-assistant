<?php
/**
 * 诊断 Archive 页面内存错误（Code Snippets 安全版本）
 * 
 * 使用方法：
...
// 防止重复加载
if (!function_exists('sosomama_add_diagnose_menu')) {
    
    // 添加管理菜单
    add_action('admin_menu', 'sosomama_add_diagnose_menu', 10);
    
    function sosomama_add_diagnose_menu() {
        // 检查函数是否已存在，避免重复添加
        if (!function_exists('sosomama_diagnose_archive_memory_page')) {
            add_management_page(
                '诊断 Archive 内存问题',
                '诊断 Archive 内存',
                'manage_options',
        ...
        <div class="wrap">
            ...
            if (isset($_GET['run_diagnosis']) && $_GET['run_diagnosis'] === '1') {
                ...
                <div class="notice notice-info">
                    ...
                    <p>
                        <a href="<?php echo esc_url(admin_url('tools.php?page=diagnose-archive-memory&run_diagnosis=1')); ?>" 
    ...
    function sosomama_run_diagnosis() {
        ...
        echo '</pre>';
        echo '<p><a href="' . esc_url(admin_url('tools.php?page=diagnose-archive-memory')) . '" class="button">返回</a></p>';
        echo '</div>';
        echo '</div>';
    }
